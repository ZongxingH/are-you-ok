...
4. Do not hard-code architecture analysis in deterministic Node code. Init only materializes the target project workspace and placeholders.
5. Use `--lang zh|en` to choose generated document language. Default is `zh`.
6. The `/auok init` skill instructs the active model session to use the internal Architect Agent for architecture analysis, including module responsibility, entrypoints, build/test strategy, and middleware/framework evidence.

...
Use the auok harness workflow in the current Codex session.
...
4. Use the environment's subagent / task / spawn_agent capability to start independent subagents:
   - Spec Agent
   - Dev Agent
   - QA Agent
   - Review Agent
...
If the current Codex environment does not support independent subagents, mark the change blocked and report that `/auok auto` requires subagent support. Do not silently downgrade to a single-agent workflow unless the user explicitly allows it.
